...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), algorithms(this) {
    ui->setupUi(this);
    InitializaAlgorithms();

    sliders = {ui->horizontalSlider, ui->horizontalSlider_3, ui->horizontalSlider_4,
...
void MainWindow::InitializaAlgorithms() {
    ...
    connect(ui->comboBox, SIGNAL(currentIndexChanged(int index)), this,
            SLOT(on_comboBox_currentIndexChanged(int index)));
}
...
void MainWindow::paintEvent(QPaintEvent *) {
    ...
    p.drawLine(pix.width() * 0.05, pix.height() / 2, pix.width() * 0.95,
               pix.height() / 2);
    p.drawLine(pix.width() / 2, pix.height() * 0.95, pix.width() / 2,
               pix.height() * 0.05);

    for (int i = 0; i < 20; i++) {
        p.setPen(QPen(Qt::black, 1));
        p.drawLine(pix.width() * (0.05 + step * i), pix.height() * 0.05,
                   pix.width() * (0.05 + step * i), pix.height() * 0.95);
        p.drawLine(pix.width() * 0.05, pix.height() * (0.05 + step * i),
                   pix.width() * 0.95, pix.height() * (0.05 + step * i));
        p.drawText(pix.width() * (0.04 + step * i), pix.height() / 1.90,
                   QString::number(range / 10 * (i - 10)));
        if (i != 10)
            p.drawText(pix.width() / 2.10, pix.height() * (0.955 - step * i),
                       QString::number(range / 10 * (i - 10)));
    }
    ...
    if (algorithm != BRESENHAM_CIRCLE) {
        for (int i = 1; i < line.size(); i++) {
            p.drawLine(pix.width() * (0.5 + stp * line[i - 1].x()),
                       pix.height() * (0.5 - stp * line[i - 1].y()),
                       pix.width() * (0.5 + stp * line[i].x()),
                       pix.height() * (0.5 - stp * line[i].y()));
        }
    } else {
        for (int i = 0; i < 4; i++) {
            for (int j = 1; j < circle[i].size(); j++) {
                p.drawLine(pix.width() * (0.5 + stp * circle[i][j - 1].x()),
                           pix.height() * (0.5 - stp * circle[i][j - 1].y()),
                           pix.width() * (0.5 + stp * circle[i][j].x()),
                           pix.height() * (0.5 - stp * circle[i][j].y()));
            }
        }
    }
...
QVector<QPoint> MainWindow::StepByStepLine(double x0, double y0, double x1,
                                           double y1) {
...
void MainWindow::on_doubleSpinBox_editingFinished() { recalc(); }

void MainWindow::on_doubleSpinBox_2_editingFinished() { recalc(); }

void MainWindow::on_doubleSpinBox_3_editingFinished() { recalc(); }

void MainWindow::on_doubleSpinBox_4_editingFinished() { recalc(); }
